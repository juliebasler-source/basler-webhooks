 ...
 * - QB now calculates line item amounts from stored Item prices (no hardcoding)
 * - Added detailed logging for invoice payload debugging
 * 
 * CHANGELOG v2.1.0:
 ...
 * Lets QuickBooks calculate line item amounts from stored Item prices.
 * 
 * @param {QuickBooks} qb - QuickBooks client
 * @param {Object} params
...
export async function createInvoice(qb, { customerId, lineItems, memo, dueDate }) {
  ...
  // IMPORTANT: Do NOT set Amount: null - this causes "failed to parse json object" error
  // Either provide a calculated Amount, or omit it entirely to let QB use the Item's price
  const lines = lineItems.map((item, index) => {
    const lineItem = {
      ...
      SalesItemLineDetail: {
        ItemRef: {
          value: String(item.itemId) // Ensure it's a string
        },
        Qty: item.quantity
      }
    };

    // Only include Amount if unitPrice is explicitly provided
    // Otherwise, let QB calculate from the Item's stored price
    if (item.unitPrice !== undefined && item.unitPrice !== null) {
      lineItem.Amount = item.quantity * item.unitPrice;
    }
  ...
  const invoiceData = {
    CustomerRef: {
      value: String(customerId) // Ensure it's a string
    },
    Line: lines,
    DueDate: dueDate,
    PrivateNote: memo,
    // NET 30 terms (value '3' is typically Net 30 in QB)
    SalesTermRef: {
      value: '3'
    }
  };
