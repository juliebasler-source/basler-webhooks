 ...
 * 
 * CHANGELOG v2.0.1:
 * - Fixed ES module syntax (import/export)
 * 
 * CHANGELOG v2.0.0:
 ...
 */
export async function findPaymentByEmail(email, lookbackMinutes = 30) {
...
/**
 * Get recent Stripe charges for debugging
 */
 ...
 * Get recent Stripe charges for debugging
 */
export async function getRecentCharges(limit = 10) {
  const stripe = new Stripe(process.env.STRIPE_SECRET_KEY);
  try {
    ...
    throw new Error(`Stripe charges lookup failed: ${err.message}`);
  }
}
